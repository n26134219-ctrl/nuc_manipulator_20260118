...
class ActionStep(BaseModel):
    ...
    mode: Optional[PickMode] = Field(None, description="抓取/放置模式")
    # 只有 pick 和 place 需要 mode，其他動作可為 None
    ...
    def to_function_call(self) -> str:
        ...
        if self.action_type == ActionType.EYE_IN_HAND:
            ...
            # 抓取動作：需要物體索引、模式、角度和手臂
            return f"pick({self.object_index}, '{self.mode.value}', {self.angle}, '{self.arm.value}')"
