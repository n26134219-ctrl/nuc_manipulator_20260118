...
class ActionStep(BaseModel):
    ...
    # Active_arm: Optional[ArmType] = Field(default=None,description="活動手臂")
    # 限制只能是 "left" 或 "right"
    ...
    def to_function_call(self) -> str:
        ...
        if self.action_type == ActionType.EYE_IN_HAND:
            ...
            return f"sweep_towards('{self.arm.value}')"
        
        elif self.action_type == ActionType.PLACE:
            # 放置動作：需要物體名稱、模式、角度和手臂
            return f"place('{self.object_name}', '{self.mode.value}', {self.angle}, '{self.arm.value}')"
            ...
            # 等待動作：需要手臂
            return f"wait('{self.arm.value}')"
        elif self.action_type == ActionType.MOVE:
            # 移動到物品位置動作：需要物體索引和手臂
            return f"move_to_object('{self.arm.value}', '{self.object_name}')"
...
class GPTPlanner:
    def __init__(self):
        ...
        self.action_description_prompt = (
            ...
            # "sweep_the_table()\n"
            "sweep_towards(arm)\n"
            "place(object_name, string place_mode, angle, arm)\n"
            "close_sandwich(arm)\n"
            "sprinkle_pepper(arm)\n"
            # "wait_for_pepper(arm)\n"
            "wait(arm)\n"
            "move_to_object(arm, object_name)\n"
            "======================================\n"
            "以下為動作函式說明:\n"
            "arm_eyeInHand_camera_catch(object_index, arm): 對應物品，分配對應手臂，讓手臂上相機再照一次，獲得準確物品資訊，更新物品資訊。【⛔嚴重警告：此函式「僅限」清掃任務(brush, dustpan)使用！製作三明治(toast)或抓取調味料時，絕對禁止呼叫此動作！】\n"
            ...
            "object_name : 要抓取的物品名稱，為字串(string)，例如 'brush' 或 'dustpan'。\n\n"
            
            # "sweep_the_table(): 使用 brush 與 dustpan 執行清掃桌動作。【⚠️ 雙手協作限制：因為是雙手動作，所以 left_arm 和 right_arm 的步驟清單中，都必須「同時包含」這個 action_type！】\n\n"
            "sweep_towards(arm): 使用指定的手臂執行掃桌動作。【⚠️ 雙手協作限制：因為是雙手動作，所以 left_arm 和 right_arm 的步驟清單中，都必須「同時包含」這個 action_type！前面一定要使用 move_to_object\n\n"
            "arm: 指定使用抓取brush的手臂清掃，選項為 'left' 或 'right'。\n\n"

            "place(object_name, string place_mode, angle, arm): 執行放置物品動作，object_name為物體名稱，place_mode為放置模式，angle為手臂角度，arm為使用的手臂(left or right)。\n"
            "object_name: 物體的名稱，為字串。\n"
            "place_mode: 放置模式，為字串(string)，包含 'down' 表示夾爪向下方放置; 'side' 表示夾爪從物品側面放置; 'forward' 表示從夾爪向前方放置。共以上三種模式"
            "angle: 物品角度，為數值。\n"
            "arm: 指定使用的手臂，選項為 'left' 或 'right'。\n\n"

            "close_sandwich(arm): 執行合起三明治動作，arm為使用的手臂(left or right)。[準備三明治或合起三明治任務時使用]\n"
            "arm: 指定使用的手臂，選項為 'left' 或 'right'。\n\n"


            "sprinkle_pepper(arm): 執行撒胡椒粉到三明治動作，arm為使用的手臂(left or right)。[準備三明治或撒胡椒粉任務時使用]\n"
            "arm: 指定使用的手臂，選項為 'left' 或 'right'。\n\n"

            "wait(arm): 執行等待動作，避免動作相撞，arm為使用的手臂(left or right)。\n"
            "arm: 指定使用的手臂，選項為 'left' 或 'right'。\n\n"

            "move_to_object(arm, object_name): 執行移動到物品位置動作。[使用 sweep_towards 或是 pick 或是 sprinkle_pepper 或是 close_sandwich 前要移動過去到目標物體，必須「包含」這個 action_type！]\n"
            "arm: 指定使用的手臂，選項為 'left' 或 'right'。\n\n"
            "object_name : 要移動到的物品名稱，為字串(string)，例如 'rice food'。\n\n"
        )
        ...
        self.safety_constraints_prompt = (
            ...
            "【視覺確認規則（重要限制）】\n"
            "1. **限定使用：** 只有在執行「清掃任務」（需要抓取掃把 brush 或畚箕 dustpan）時，才必須在 `pick` 之前先執行 `arm_eyeInHand_camera_catch` 進行重新拍攝定位。\n"
            "2. **禁止使用：** 若是執行抓取吐司 toast、胡椒粉 pepper shaker 等其他物品），**絕對不需要且禁止**在抓取前執行 `arm_eyeInHand_camera_catch`，請直接規劃 `pick` 動作即可。\n"
            
            "【絕對時序與協作規則（極重要）】\n"
            "1. 製作三明治的絕對順序：必須是『先撒胡椒粉 (sprinkle_pepper)』，完成後才能『蓋上吐司 (close_sandwich)』！絕對不可顛倒！\n"
            "2. 雙手時間軸對齊：如果右手負責撒胡椒粉，左手負責蓋吐司，左手必須先規劃 `wait` 動作來等待右手，確保 `close_sandwich` 的步驟編號 (step_id) 大於 `sprinkle_pepper` 的步驟編號，並在 prerequisites 加上對應的依賴。\n"
            "3. 空間防撞：當兩隻手都要靠近同一個物體 (例如 sandwich) 時，不可在同一個 step_id 執行 move_to_object，必須一先一後（其中一隻手先 wait）。\n"
            "4. 空間防撞2: 當其中一隻手在撒胡椒粉 (sprinkle_pepper) 時，另一隻手不可同時在 move_to_object 接近三明治，必須先 wait 等撒完再 move_to_object, wait 可以使用不只一次。"
            "5. 清掃協作：如果任務需要同時使用掃把和畚箕進行清掃，則 left_arm 和 right_arm 的步驟清單中，都必須「同時包含」sweep_towards 這個 action_type，且前面兩手都一定要使用 move_to_object 移動到目標物體位置！\n"
        )
        self.task_planning_profile_prompt = (
            ...
            "動作規劃: "
            "left arm:"
            "Step 1. move_to_object('left', 'pepper shaker')"
            "Step 2. pick('left', 'pepper shaker')"
            "==============================="
            "任務規劃範例三: "
            "==============================="
            "任務: 抓取胡椒粉"
            "動作規劃: "
            "right arm:"
            "Step 1. move_to_object('right', 'pepper shaker')"
            "Step 2. pick('right', 'pepper shaker')"
            # "==============================="
            # "任務規劃範例四: "
            # "==============================="
            # "任務: 灑胡椒粉並將土司覆蓋三明治來完成製作三明治（假設左手已持有胡椒粉，右手抓取吐司）"
            # "動作規劃: "
            # "left arm:"
            # "Step 1. move_to_object('left', 'sandwich')"
            # "Step 2. sprinkle_pepper('left')"
            # "right arm:"
            # "Step 1. move_to_object('right', 'sandwich')"
            # "Step 2. wait_for_pepper('right')"
            # "Step 3. close_sandwich('right')"

            "==============================="
            "任務規劃範例五: "
            "==============================="
            "任務: 抓取吐司(假設左手已持有胡椒粉，右手空閒）"
            "動作規劃: "
            "right arm:"
            "Step 1. move_to_object('right', 'sliced toast')"
            "Step 2. pick('right', 'sliced toast')"
            
            "==============================="
            "任務規劃範例六: "
            "==============================="
            "任務: 抓取吐司(假設左手空閒，右手已持有胡椒粉）"
            "動作規劃: "
            "left arm:"
            "Step 1. move_to_object('left', 'sliced toast')"
            "Step 2. pick('left', 'sliced toast')"
            # "==============================="
            # "任務規劃範例七: "
            # "==============================="
            ...
            "動作規劃: "
            "left arm:"
            "Step 1. place('pepper shaker', 'side', 150, 'left')"
        ...
        self.output_format_prompt = (
            ...
            "  \"left_arm\": [\n"
            "    {\n"
            "      \"step_id\": <步驟編號，正整數。⚠️【極重要限制】左手 (left_arm) 與右手 (right_arm) 的 step_id 必須「各自獨立」計算！左手必須從 1, 2, 3... 連續遞增，右手也必須從 1, 2, 3... 連續遞增。絕對不可雙手共用編號或跳號！>,\n"
            ...
            "      \"prerequisites\": [<只看左手前置步驟ID列表，整數陣列>]\n"
            ...
            "      \"mode\": \"<模式>\",\n"
            "      \"angle\": <角度>,\n"
            "      \"prerequisites\": [<只看右手前置步驟ID列表，整數陣列>]\n"
            "      \"object_name\": \"<物品名稱>\",\n"
            "    }\n"
            "  ]\n"
            "}\n"
            ...
            "- `prerequisites`: 前置步驟ID陣列，表示此步驟依賴哪些步驟完成，不能依賴自己當下步驟。若無依賴則使用空陣列 []\n"
            "\n"
            ...
            # "```json\n"
            # "{\n"
            # "  \"task_description\": \"灑胡椒粉並將土司覆蓋三明治來完成製作三明治（假設左手已持有胡椒粉，右手抓取吐司）\",\n"
            # "  \"left_arm\": [\n"
            # "    {\n"
            # "      \"step_id\": 1,\n"
            # "      \"action_type\": \"move_to_object\",\n"
            ...
            # "}\n"
            # "```\n"
            "**範例任務五：抓取吐司(假設左手已持有胡椒粉，右手空閒）**\n"
            "\n"
            "```json\n"
            "{\n"
            "  \"task_description\": \"抓取吐司(假設左手已持有胡椒粉，右手空閒）\",\n"
            "  \"left_arm\": [\n"
            "  ],\n"
            "  \"right_arm\": [\n"
            "    {\n"
            ...
            "}\n"
            "```\n"
            "**範例任務六：抓取吐司(假設左手空閒，右手已持有胡椒粉）**\n"
            "\n"
            "```json\n"
            "{\n"
            "  \"task_description\": \"抓取吐司(假設左手空閒，右手已持有胡椒粉）\",\n"
            "  \"left_arm\": [\n"
            "    {\n"
            "      \"step_id\": 1,\n"
            "      \"action_type\": \"move_to_object\",\n"
...
# 2. 定義多段 User Prompt 組合函式
    def create_robot_planning_messages( self):
        ...
        if self.error_feedback_prompt:
            self.sections.append(
                f"## ⚠️ 錯誤修正要求 (前次生成的計畫未通過物理/邏輯驗證)\n"
                f"{self.error_feedback_prompt}\n"
                f"請務必檢視上述錯誤，並在這次生成的計畫中完美修正！"
            )
